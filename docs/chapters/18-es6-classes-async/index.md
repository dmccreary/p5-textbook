...
!!! mascot-welcome "Welcome to Chapter 18"
    ...
    Welcome back, artists! Ready to build complex, multi-layered masterpieces without losing your mind? This chapter unlocks the ultimate architectural secret of modern coding: creating reusable blueprints for your digital objects. Time to color outside the loops and level up!
...
!!! mascot-thinking "Thinking About Classes"
    ...
    Think about it like this: before classes, our sketches were like painting directly on the canvas. With classes, we're building custom stamp factories! You design the stamp once (the class), and then you can stamp it a hundred times (the objects) with different properties.
...
!!! mascot-tip "A Helpful Tip on Spread"
    ...
    Here's a secret: you can use the spread operator to quickly clone an array without referencing the original! Instead of `let newArr = oldArr`, which links them, use `let newArr = [...oldArr]`. Now you can safely modify the copy!
...
!!! mascot-warning "A Common Mistake with Await"
    ...
    Watch out for the "SyntaxError: await is only valid in async functions" trap! This happens when you try to `await` data without tagging the parent function. To fix it, just add the `async` keyword right before `function` in your definition!
...
!!! mascot-celebration "Chapter Complete"
    ...
    Incredible work! You just mastered destructuring, higher-order array methods, and fetching asynchronous JSON data. Your toolkit is now packed with the power of modern ES6! Time to color outside the loops!
