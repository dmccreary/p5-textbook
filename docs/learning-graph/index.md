...
We use generative AI to convert the course description into a [Concept List](./concept-list.md). Each concept is in the form of a short Title Case label with most labels under 32 characters long. This graph features **600 concepts** covering 2D/3D graphics, math, animation, physics, ES6+ JavaScript, `p5.sound`, microphone input, FFT signal processing, and computational thinking pedagogy.
...
- Validates course description has sufficient depth for generating 600 concepts
...
In order to see patterns in the learning graph, it is useful to assign colors to each concept based on the concept type. We use generative AI to create 18 balanced categories for our concepts and then place each concept into a single primary classifier.

- A concept classifier taxonomy with 18 categories
- Category organization - foundational elements first, course capstone project ideas last
- Balanced categories (5% - 6% each)
...
This report shows how many concepts fit into each category of the taxonomy. Our goal is a balanced taxonomy where each category holds a well-proportioned number of concepts without exceeding the 30% threshold.
