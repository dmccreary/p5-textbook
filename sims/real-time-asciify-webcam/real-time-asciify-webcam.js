/* Real-Time ASCIIfy Camera MicroSim
   ...
   For a complete lesson plan see:  https://dmccreary.github.io/p5-textbook/sims/real-time-asciify-webcam/
*/
// CANVAS_HEIGHT: 485
let canvasWidth = 400;
let drawHeight = 400;
...
function setup() {
  ...
  video.size(canvasWidth, drawHeight);
  video.hide();
  ...
  startBtn.mousePressed(() => {
    isRunning = !isRunning;
    startBtn.html(isRunning ? 'Pause' : 'Start Camera');
  });
  positionControls();
  describe('ASCII art converter mapping webcam pixel luminance to text characters.', FALLBACK);
}
...
function draw() {
  ...
  // Render Webcam Visual Scene to Sample Grayscale
  fill(0, 255, 120);
  noStroke();
  textSize(gridSize);
  ...
  if (video.pixels && video.pixels.length > 0) {
    for (let x = gridSize / 2; x < canvasWidth; x += gridSize) {
      for (let y = 40 + gridSize / 2; y < drawHeight - 10; y += gridSize) {
        // Calculate brightness from webcam pixels
        // Map grid coordinates to video coordinates
        let px = floor(map(x, 0, canvasWidth, 0, video.width));
        let py = floor(map(y, 0, drawHeight, 0, video.height));
        ...
        let charIndex = floor(map(bright, 0, 255, 0, rampChars.length - 1));
        // Constrain index to avoid out of bounds
        charIndex = constrain(charIndex, 0, rampChars.length - 1);
  ...
  textFont('Arial');
  text('Real-Time ASCIIfy Camera', canvasWidth / 2, 12);
